...
public class NastavitveFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        /*
        JobScheduler jobScheduler = (JobScheduler) getSystemService(getContext().JOB_SCHEDULER_SERVICE);

        ComponentName componentName = new ComponentName(this, syncData.class);
        ...
        Button bVzdrzevanjeDN = (Button) v_nastavitve.findViewById(R.id.bPrenesiVZDN);
        bVzdrzevanjeDN.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                pbPrenos.setVisibility(View.VISIBLE);
                new LoadVzdrzevanjeDNFromWeb().execute();
    ...
    private class LoadVzdrzevanjeDNFromWeb extends AsyncTask {
            ...
            if (result != null)
            {
                ...
                try {
                    ...
                    for (int i=0; i < dataArray.length() ; i++)
                    {
                        ...
                        String ime_vira = object.getString("ime_vira");

                        DatabaseHandler db = new DatabaseHandler(getContext());
                        db.insertUpdateTehnik(tehnik_id,naziv,servis, montaza, vzdrzevanje, ime_regala, user_id,ime_vira);
                    }
                    //getString("user_id");//.getString("user_id");
                } catch (JSONException e) {
                    e.printStackTrace();
    ...
    private class LoadUporabnikiFromWeb extends AsyncTask {
        ...
        protected Object doInBackground(Object... arg0) {
            ...
            result = null;
            try {
                URL url = new URL("https://www.sintal.si/tehnika/getUsersPodjetje.php");
                HttpURLConnection conn4 = (HttpURLConnection) url.openConnection();
                String username ="sintal_teh";
                String password = "mCuSTArQ*PdWAH#7-getUsersPodjetje";
                String userpass = username + ":" + password;
                String basicAuth = "Basic " + new String(Base64.getEncoder().encode(userpass.getBytes()));
                conn4.setRequestProperty ("Authorization", basicAuth);
                conn4.connect();
            ...
            if (result != null)
            {
                ...
                try {
                    JSONArray dataArray = obj.getJSONArray("users_podjetje");
                    //int pageName = obj.getJSONArray("users").length();
                    DatabaseHandler db = new DatabaseHandler(getContext());
                    db.deleteUpdateUpoPodjetje();
                    for (int i=0; i < dataArray.length() ; i++)
                    {
                        JSONObject object = dataArray.getJSONObject(i);
                        String user_id = object.getString("user_id");
                        ...
                        db.insertUpdateupoPodjejte(user_id,podjetje);
                    }
                    //getString("user_id");//.getString("user_id");
                } catch (JSONException e) {
                    e.printStackTrace();
